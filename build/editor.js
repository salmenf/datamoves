...
import {customElement, property, query, state, queryAll} from "./_snowpack/pkg/lit/decorators.js";
...
import {LABELS, TEXT_PATTERNS} from "./localization.js";
...
const castValue = (value) => {
  if (value?.type === "DataFrame") {
    let records = JSON.parse(value.to_json(void 0, "records"));
    return records.map((entry) => Object.fromEntries(Object.entries(entry).map(([key, value2]) => [key.replace(/\./g, "․"), value2])));
  } else {
    return value;
  }
};
export let PythonOperationSequenceEditor = class extends LitElement {
  constructor() {
    super(...arguments);
    this.explainer = html``;
    this.statements = [];
    this.scenarioContinueSelector = null;
    this.datasetsHidden = false;
    this.scenarioStep = -1;
    this.scenarioStopped = false;
    this.querySelectorAllNodes = (query2) => {
      return [
        ...this.shadowRoot.querySelectorAll(query2),
        ...[...this.operationNodes].flatMap((n) => [...n.shadowRoot.querySelectorAll(query2)])
      ];
    };
    this.getOperationNode = (i) => {
      return this.shadowRoot.querySelector(`[id='${i}']`);
    };
    this.resetScenario = () => {
      this.explainer = null;
      this.statements = [];
    ...
    this.statementToNode = (statement, i) => {
    ...
    this.skipButton = (showText = false, disabled = true) => {
    ...
    this.playButton = (showText = false) => {
  ...
  async updated(changedProperties) {
    ...
    if (this.scenarioContinueSelector) {
      const continueElements = this.querySelectorAllNodes(this.scenarioContinueSelector).flatMap((el) => ["identifier", "expression"].includes(el["name"]) ? [el, el.nextElementSibling] : [el]);
      for (const continueElement of continueElements) {
        "disabled" in continueElement ? continueElement.disabled = false : null;
    ...
    if (changedProperties.has("scenarioContinueSelector")) {
      const query2 = changedProperties.get("scenarioContinueSelector");
      const oldContinueElements = this.querySelectorAllNodes(query2).flatMap((el) => ["identifier", "expression"].includes(el["name"]) ? [el, el.nextElementSibling] : [el]);
      for (const oldContinueElement of oldContinueElements) {
        "disabled" in oldContinueElement ? oldContinueElement.disabled = true : null;
  ...
  insertStatement(statement, i = this.statements.length - 1) {
    const statements = [...this.statements];
    statements.splice(i + 1, 0, statement);
    this.statements = statements;
  }
  get statementList() {
    return this.statements.length > 0 ? this.statements.map(this.statementToNode) : this.insertStatementButton();
  }
  get resetButton() {
...
PythonOperationSequenceEditor.styles = css`
    :host, #statements {
      display: flex;
...
], PythonOperationSequenceEditor.prototype, "operationNodes", 2);
__decorate([
  property({attribute: false})
], PythonOperationSequenceEditor.prototype, "pyodide", 2);
__decorate([
  property()
], PythonOperationSequenceEditor.prototype, "lang", 2);
__decorate([
  property({attribute: false})
], PythonOperationSequenceEditor.prototype, "explainer", 2);
__decorate([
  property({attribute: false})
], PythonOperationSequenceEditor.prototype, "statements", 2);
__decorate([
  property({attribute: false})
], PythonOperationSequenceEditor.prototype, "scenario", 2);
__decorate([
  property()
], PythonOperationSequenceEditor.prototype, "scenarioContinueSelector", 2);
__decorate([
  property({type: Boolean})
], PythonOperationSequenceEditor.prototype, "datasetsHidden", 2);
__decorate([
  state()
], PythonOperationSequenceEditor.prototype, "scenarioStep", 2);
__decorate([
  state()
], PythonOperationSequenceEditor.prototype, "scenarioStopped", 2);
PythonOperationSequenceEditor = __decorate([
  customElement("dm-operation-sequence-editor")
], PythonOperationSequenceEditor);
export let PythonOperationNode = class extends LitElement {
  constructor() {
    super(...arguments);
    this.identifier = "";
    this.expression = "";
    ...
    this.recognizedPresetName = "none";
    ...
    this.identifierFieldJump = (e) => {
      if (!this.expression && (e.key === "ArrowRight" || e.key === "Enter") && e.currentTarget.selectionStart === this.identifier.length) {
    ...
    this.expressionBlurTimeout = null;
  }
  ...
  get code() {
    return this.identifier && this.expression ? this.identifier + " = " + this.expression : "";
  }
  get identifierFieldSize() {
    return Math.max(this.identifier.length, this.identifierPlaceholder.length, ...this.presets.map((p) => p.name.length));
  }
  get expressionFieldSize() {
    return this.expression.length > this.expressionPlaceholder.length ? this.expression.length : this.expressionPlaceholder.length;
  }
  focus() {
    this.identifierElement.focus();
  }
  get matchingPreset() {
    return this.presets.filter((p) => p.pattern.test(this.expression)).sort((a, b) => a.pattern.source.length - b.pattern.source.length).at(-1);
  }
  get PresetPicker() {
  ...
  get resultView() {
    if (typeof this.result !== "object") {
      ...
      const columns = this.result.length ? Object.keys(this.result[0]).map((key) => html`<vaadin-grid-column path=${key} auto-width></vaadin-grid-column>`) : null;
      return html`
  ...
  get expressionInput() {
    return this.dynamicStyles.length === 1 || this.expressionActive ? html`
        <input
          class="expression"
  ...
  get identifierInputStyles() {
    const s = this.dynamicStyles.find((s2) => s2.identifierPattern);
  ...
  render() {
    ...
    return html`
      ...
      <form tabIndex=${this.disabled ? -1 : void 0} @submit=${(e) => e.preventDefault()} class="main" id=${"main" + this.id} @keydown=${this.jump} @focusout=${(e) => {
      ...
      e.stopPropagation();
        ...
        <button
          ...
          title=${!this.code ? LABELS[this.lang]["noIdentifierOrExpression"] : !this.pyodide ? LABELS[this.lang]["loadingPyodide"] : LABELS[this.lang]["runCode"]}>
...
PythonOperationNode.shadowRootOptions = {...LitElement.shadowRootOptions, mode: "open"};
PythonOperationNode.styles = css`
...
], PythonOperationNode.prototype, "identifier", 2);
__decorate([
  property({type: String})
], PythonOperationNode.prototype, "expression", 2);
__decorate([
  property({attribute: false})
], PythonOperationNode.prototype, "pyodide", 2);
__decorate([
  property({type: String})
], PythonOperationNode.prototype, "lang", 2);
__decorate([
  property({attribute: false})
], PythonOperationNode.prototype, "info", 2);
__decorate([
  property({type: Boolean})
], PythonOperationNode.prototype, "disabled", 2);
__decorate([
  property({type: Boolean})
], PythonOperationNode.prototype, "alwaysShowResult", 2);
__decorate([
  property({attribute: false})
], PythonOperationNode.prototype, "dynamicStyles", 2);
__decorate([
  state()
], PythonOperationNode.prototype, "expressionActive", 2);
__decorate([
  state()
], PythonOperationNode.prototype, "pyodideError", 2);
__decorate([
  state()
], PythonOperationNode.prototype, "selectedPresetName", 2);
__decorate([
  state()
], PythonOperationNode.prototype, "recognizedPresetName", 2);
__decorate([
  state()
], PythonOperationNode.prototype, "result", 2);
__decorate([
  query("input.expression")
], PythonOperationNode.prototype, "expressionElement", 2);
__decorate([
  query("input.identifier")
], PythonOperationNode.prototype, "identifierElement", 2);
__decorate([
  query("form")
], PythonOperationNode.prototype, "formElement", 2);
PythonOperationNode = __decorate([
